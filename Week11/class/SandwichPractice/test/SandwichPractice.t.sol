...
contract SandwichPracticeTest is SandwichSetUp {
    ...
    function _attackerAction1() internal {
        ...
        console.log("attackerUsdcBalance",attackerUsdcBalance);

        vm.stopPrank();

        // // check victim usdc balance >= originalUsdcAmountOutMin (93780012)
        // assertGe(usdc.balanceOf(attacker), originalUsdcAmountOutMin);
    }
    ...
    function _attackerAction2() internal {
        ...
        uint256 attackerUsdcBalance = usdc.balanceOf(address(attacker));
        
        usdc.approve(address(uniswapV2Router), attackerUsdcBalance);

        uint256 amountOut = uniswapV2Router.getAmountsOut(attackerUsdcBalance, path)[0];
